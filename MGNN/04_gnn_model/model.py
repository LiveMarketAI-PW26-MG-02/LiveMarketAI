#!/usr/bin/env python3
"""
Microstructure Graph Neural Network (MGNN) - GNN Model — model.py Entry
Implements the MicrostructureGNN graph neural network architecture
...
class GraphConvLayer:
    ...
    Used as a base building block in MGNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for MGNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of MGNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in MGNN.
    """
...
class MicrostructureGNN:
    """
    Microstructure Graph Neural Network (MGNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the MGNN GNN.
...
def build_model(config: Optional[dict] = None) -> MicrostructureGNN:
    """Factory function to build MGNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = MicrostructureGNN(default_config)
    print(f"  Built MGNN model with config: {default_config}")
    return model
...
def main():
    ...
    print(f"\nMGNN GNN Model Module Complete.\n")
    return model, output
