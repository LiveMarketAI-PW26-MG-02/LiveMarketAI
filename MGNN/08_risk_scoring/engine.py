#!/usr/bin/env python3
"""
Microstructure Graph Neural Network (MGNN) - Risk Scoring Engine (Alt Entry)
Computes microstructure_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/mgnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for MGNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: MGNN | Primary risk metric: microstructure_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["microstructure_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["mgnn_severity"] = pd.cut(
            result["microstructure_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["mgnn_label"] = (result["microstructure_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for MGNN."""
...
def main():
    ...
    print(f"  Mean {'microstructure_manipulation_score'}: {scored_df['microstructure_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['microstructure_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'microstructure_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["microstructure_manipulation_score"], "severity": r["mgnn_severity"], "module": "MGNN"}
        for i, r in scored_df[scored_df["mgnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"MGNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nMGNN Risk Scoring Complete.\n")
