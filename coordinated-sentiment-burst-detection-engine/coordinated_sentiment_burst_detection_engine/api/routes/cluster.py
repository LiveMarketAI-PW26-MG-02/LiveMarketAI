...
from ...schemas.cluster import ClusterCreate, ClusterRead

router = APIRouter(prefix="/clusters", tags=["Cluster"])
...
@router.get("/{item_id}", response_model=ClusterRead)
...
def create_item(payload: ClusterCreate, db: Session = Depends(get_db)):
