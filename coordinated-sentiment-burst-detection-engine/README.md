# Coordinated Sentiment-Burst Detection Engine

Detects synchronised, coordinated sentiment spikes across sources and explains the coordination evidence.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /posts, /burst_signals, /sources, /clusters, /detection_alerts
- **AI primitive**: `nlp` — see `coordinated_sentiment_burst_detection_engine/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn coordinated_sentiment_burst_detection_engine.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    coordinated_sentiment_burst_detection_engine/
