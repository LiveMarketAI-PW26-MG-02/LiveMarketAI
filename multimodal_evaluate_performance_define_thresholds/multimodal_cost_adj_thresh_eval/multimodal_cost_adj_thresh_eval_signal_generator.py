"""
  ...
  Folder     : multimodal_cost_adj_thresh_eval
  File       : multimodal_cost_adj_thresh_eval_signal_generator.py
  Purpose    : Evaluate Performance — Cost Adj Thresh Eval
"""
...
MULTIMODAL_MODULE = 'multimodal_cost_adj_thresh_eval_signal_generator'
...
# ── Configuration ─────────────────────────────────────────────────────────────
@dataclass
class MultimodalCostAdjThreshEvalSignalGeneratorConfig:
    """Multimodal threshold-sweep config for cost_adj_thresh_eval."""
...
# ── Core Engine ───────────────────────────────────────────────────────────────
class MultimodalCostAdjThreshEvalEngine(MultimodalThresholdBase):
    """
    Multimodal threshold engine: evaluate performance — cost adj thresh eval.
    Sweeps, evaluates, validates and documents signal thresholds.
    """
    def __init__(self, cfg: Optional[MultimodalCostAdjThreshEvalSignalGeneratorConfig] = None):
        self.cfg      = cfg or MultimodalCostAdjThreshEvalSignalGeneratorConfig()
