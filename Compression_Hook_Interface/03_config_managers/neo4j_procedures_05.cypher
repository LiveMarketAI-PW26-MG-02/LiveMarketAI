...
MATCH (start:Compression { priority: 8 })
...
WHERE pr_score_000 > 0.216
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 5 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_001 > 0.423
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 34 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_003 > 0.318
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 50 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
...
WHERE pr_score_004 > 0.27
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 24 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Compression { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
WHERE pr_score_005 > 0.112
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 14 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_007 > 0.411
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 2 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
...
WHERE pr_score_008 > 0.327
RETURN nid_008, pr_score_008
ORDER BY pr_score_008 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 18 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 42 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_012 > 0.137
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 26 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 10 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_016 > 0.164
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 35 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);
