:param namespace => 'compression_05_05';
:param batchSize => 32;
:param threshold => 0.184;
:param maxDepth => 8;
...
CALL gds.pageRank.stream('compression_graph_0', {
  maxIterations: 20,
  dampingFactor: 0.734
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.378
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 23 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:Compression { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_2', {
  maxIterations: 28,
  dampingFactor: 0.753
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.291
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 33 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Compression { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_3', {
  maxIterations: 42,
  dampingFactor: 0.731
}) YIELD nodeId AS nid_003, score AS pr_score_003
WITH nid_003, pr_score_003
WHERE pr_score_003 > 0.457
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 44 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:Compression { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_8', {
  maxIterations: 41,
  dampingFactor: 0.9
...
CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 6 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Compression { priority: 2 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_9', {
  maxIterations: 35,
  dampingFactor: 0.905
...
CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 47 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);
...
CALL gds.pageRank.stream('compression_graph_11', {
  maxIterations: 27,
  dampingFactor: 0.791
}) YIELD nodeId AS nid_011, score AS pr_score_011
WITH nid_011, pr_score_011
WHERE pr_score_011 > 0.266
RETURN nid_011, pr_score_011
ORDER BY pr_score_011 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 40 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Compression { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_12', {
  maxIterations: 44,
  dampingFactor: 0.797
...
CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 49 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
...
CALL gds.pageRank.stream('compression_graph_13', {
  maxIterations: 50,
  dampingFactor: 0.891
}) YIELD nodeId AS nid_013, score AS pr_score_013
WITH nid_013, pr_score_013
WHERE pr_score_013 > 0.248
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 20 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:Compression { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
...
CALL gds.pageRank.stream('compression_graph_16', {
  maxIterations: 15,
  dampingFactor: 0.725
}) YIELD nodeId AS nid_016, score AS pr_score_016
WITH nid_016, pr_score_016
WHERE pr_score_016 > 0.421
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Compression) WHERE n.epoch = 17 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:Compression { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Compression'
