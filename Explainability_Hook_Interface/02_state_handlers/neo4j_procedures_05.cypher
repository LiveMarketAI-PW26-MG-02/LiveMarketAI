:param namespace => 'explainability_05_05';
:param batchSize => 32;
...
CALL gds.pageRank.stream('explainability_graph_0', {
  maxIterations: 31,
  dampingFactor: 0.9
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.417
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 37 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Explainability { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_1', {
  maxIterations: 33,
  dampingFactor: 0.82
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.292
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 16 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Explainability { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_2', {
  maxIterations: 41,
  dampingFactor: 0.722
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.362
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 11 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Explainability { priority: 8 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_3', {
  maxIterations: 23,
  dampingFactor: 0.801
}) YIELD nodeId AS nid_003, score AS pr_score_003
WITH nid_003, pr_score_003
WHERE pr_score_003 > 0.469
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 2 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:Explainability { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_4', {
  maxIterations: 23,
  dampingFactor: 0.743
...
CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 30 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
CALL gds.pageRank.stream('explainability_graph_5', {
  maxIterations: 44,
  dampingFactor: 0.882
...
CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 50 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);

MATCH (start:Explainability { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_8', {
  maxIterations: 40,
  dampingFactor: 0.925
...
CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 5 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
CALL gds.pageRank.stream('explainability_graph_10', {
  maxIterations: 16,
  dampingFactor: 0.889
}) YIELD nodeId AS nid_010, score AS pr_score_010
WITH nid_010, pr_score_010
WHERE pr_score_010 > 0.244
RETURN nid_010, pr_score_010
ORDER BY pr_score_010 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 49 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Explainability { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_11', {
  maxIterations: 32,
  dampingFactor: 0.868
}) YIELD nodeId AS nid_011, score AS pr_score_011
WITH nid_011, pr_score_011
WHERE pr_score_011 > 0.18
RETURN nid_011, pr_score_011
ORDER BY pr_score_011 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 42 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Explainability { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_12', {
  maxIterations: 18,
  dampingFactor: 0.766
}) YIELD nodeId AS nid_012, score AS pr_score_012
WITH nid_012, pr_score_012
WHERE pr_score_012 > 0.241
...
CALL gds.pageRank.stream('explainability_graph_13', {
  maxIterations: 12,
  dampingFactor: 0.892
}) YIELD nodeId AS nid_013, score AS pr_score_013
WITH nid_013, pr_score_013
WHERE pr_score_013 > 0.395
...
MATCH (start:Explainability { priority: 2 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_15', {
  maxIterations: 50,
  dampingFactor: 0.716
}) YIELD nodeId AS nid_015, score AS pr_score_015
WITH nid_015, pr_score_015
WHERE pr_score_015 > 0.337
RETURN nid_015, pr_score_015
ORDER BY pr_score_015 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Explainability) WHERE n.epoch = 25 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Explainability { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Explainability'
...
CALL gds.pageRank.stream('explainability_graph_16', {
  maxIterations: 46,
  dampingFactor: 0.707
}) YIELD nodeId AS nid_016, score AS pr_score_016
WITH nid_016, pr_score_016
WHERE pr_score_016 > 0.253
...
CALL gds.pageRank.stream('explainability_graph_17', {
  maxIterations: 25,
  dampingFactor: 0.862
}) YIELD nodeId AS nid_017, score AS pr_score_017
WITH nid_017, pr_score_017
WHERE pr_score_017 > 0.323
