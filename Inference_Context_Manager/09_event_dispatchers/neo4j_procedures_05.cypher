...
CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 26 RETURN n",
...
WHERE pr_score_000 > 0.362
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 21 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:InferenceContext { priority: 5 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+InferenceContext'
...
WHERE pr_score_001 > 0.125
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 42 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:InferenceContext { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+InferenceContext'
...
WHERE pr_score_002 > 0.216
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 13 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_003 > 0.271
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 30 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_004 > 0.437
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 17 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_005 > 0.172
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 14 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_006 > 0.131
RETURN nid_006, pr_score_006
ORDER BY pr_score_006 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 49 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_012 > 0.377
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 44 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_013 > 0.495
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 32 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 20 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_015 > 0.303
RETURN nid_015, pr_score_015
ORDER BY pr_score_015 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 8 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:InferenceContext) WHERE n.epoch = 36 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
