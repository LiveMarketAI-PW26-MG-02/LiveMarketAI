#!/usr/bin/env python3
"""
Counterfactual Graph Neural Network (CGNN-XAI) - Risk Scoring Engine (Alt Entry)
Computes counterfactual_risk_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/cgnn_xai_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for CGNN-XAI manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: CGNN-XAI | Primary risk metric: counterfactual_risk_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["counterfactual_risk_score"] = np.clip(ensemble, 0, 1)
        result["cgnn_xai_severity"] = pd.cut(
            result["counterfactual_risk_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["cgnn_xai_label"] = (result["counterfactual_risk_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for CGNN-XAI."""
...
def main():
    ...
    print(f"  Mean {'counterfactual_risk_score'}: {scored_df['counterfactual_risk_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['counterfactual_risk_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'counterfactual_risk_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["counterfactual_risk_score"], "severity": r["cgnn_xai_severity"], "module": "CGNN-XAI"}
        for i, r in scored_df[scored_df["cgnn_xai_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"CGNN-XAI","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nCGNN-XAI Risk Scoring Complete.\n")
