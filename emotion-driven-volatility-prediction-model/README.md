# Emotion-Driven Volatility Prediction Model

Forecasts volatility from aggregated market-emotion signals with transparent per-feature contributions.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /emotion_signals, /volatility_forecasts, /sources, /feature_contributions, /snapshots
- **AI primitive**: `forecast` — see `emotion_driven_volatility_prediction_model/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn emotion_driven_volatility_prediction_model.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    emotion_driven_volatility_prediction_model/
