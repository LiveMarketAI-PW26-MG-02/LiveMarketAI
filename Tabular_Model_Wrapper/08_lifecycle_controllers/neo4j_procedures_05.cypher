...
WHERE pr_score_005 > 0.357
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:TabularModel) WHERE n.epoch = 3 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_008 > 0.383
RETURN nid_008, pr_score_008
ORDER BY pr_score_008 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:TabularModel) WHERE n.epoch = 50 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_010 > 0.331
RETURN nid_010, pr_score_010
ORDER BY pr_score_010 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:TabularModel) WHERE n.epoch = 44 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
