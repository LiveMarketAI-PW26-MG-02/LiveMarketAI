...
WHERE pr_score_000 > 0.167
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 13 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 2 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
WHERE pr_score_003 > 0.374
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 29 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_004 > 0.273
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 5 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_005 > 0.248
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 21 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_007 > 0.416
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 15 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
WHERE pr_score_008 > 0.124
RETURN nid_008, pr_score_008
ORDER BY pr_score_008 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 35 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
WHERE pr_score_009 > 0.243
RETURN nid_009, pr_score_009
ORDER BY pr_score_009 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 1 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:Transformer { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Transformer'
...
WHERE pr_score_010 > 0.274
RETURN nid_010, pr_score_010
ORDER BY pr_score_010 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 32 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:Transformer { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Transformer'
...
CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 28 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 10 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_015 > 0.34
RETURN nid_015, pr_score_015
ORDER BY pr_score_015 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 47 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_016 > 0.254
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Transformer) WHERE n.epoch = 3 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
