:param namespace => 'graphnetwork_05_05';
:param batchSize => 256;
:param threshold => 0.151;
:param maxDepth => 12;
...
CALL gds.pageRank.stream('graphnetwork_graph_0', {
  maxIterations: 32,
  dampingFactor: 0.921
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.177
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 23 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:GraphNetwork { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_1', {
  maxIterations: 21,
  dampingFactor: 0.94
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.387
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 28 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:GraphNetwork { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_2', {
  maxIterations: 33,
  dampingFactor: 0.872
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.392
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 14 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:GraphNetwork { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_4', {
  maxIterations: 27,
  dampingFactor: 0.76
}) YIELD nodeId AS nid_004, score AS pr_score_004
WITH nid_004, pr_score_004
WHERE pr_score_004 > 0.398
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 48 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:GraphNetwork { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_6', {
  maxIterations: 50,
  dampingFactor: 0.739
}) YIELD nodeId AS nid_006, score AS pr_score_006
WITH nid_006, pr_score_006
WHERE pr_score_006 > 0.266
RETURN nid_006, pr_score_006
ORDER BY pr_score_006 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 4 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:GraphNetwork { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_11', {
  maxIterations: 26,
  dampingFactor: 0.793
...
MATCH (start:GraphNetwork { priority: 5 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_14', {
  maxIterations: 40,
  dampingFactor: 0.725
}) YIELD nodeId AS nid_014, score AS pr_score_014
WITH nid_014, pr_score_014
WHERE pr_score_014 > 0.365
...
MATCH (start:GraphNetwork { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_16', {
  maxIterations: 38,
  dampingFactor: 0.926
}) YIELD nodeId AS nid_016, score AS pr_score_016
WITH nid_016, pr_score_016
WHERE pr_score_016 > 0.23
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:GraphNetwork) WHERE n.epoch = 29 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:GraphNetwork { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+GraphNetwork'
...
CALL gds.pageRank.stream('graphnetwork_graph_17', {
  maxIterations: 24,
  dampingFactor: 0.713
}) YIELD nodeId AS nid_017, score AS pr_score_017
WITH nid_017, pr_score_017
WHERE pr_score_017 > 0.289
