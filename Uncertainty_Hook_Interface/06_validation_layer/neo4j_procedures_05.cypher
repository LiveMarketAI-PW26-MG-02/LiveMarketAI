:param namespace => 'uncertainty_05_05';
:param batchSize => 32;
...
CALL gds.pageRank.stream('uncertainty_graph_0', {
  maxIterations: 10,
  dampingFactor: 0.888
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.108
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 47 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Uncertainty { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_1', {
  maxIterations: 24,
  dampingFactor: 0.809
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.293
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 24 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:Uncertainty { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_4', {
  maxIterations: 31,
  dampingFactor: 0.77
}) YIELD nodeId AS nid_004, score AS pr_score_004
WITH nid_004, pr_score_004
WHERE pr_score_004 > 0.143
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 37 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Uncertainty { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_6', {
  maxIterations: 14,
  dampingFactor: 0.859
}) YIELD nodeId AS nid_006, score AS pr_score_006
WITH nid_006, pr_score_006
WHERE pr_score_006 > 0.398
RETURN nid_006, pr_score_006
ORDER BY pr_score_006 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 6 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Uncertainty { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_7', {
  maxIterations: 25,
  dampingFactor: 0.825
}) YIELD nodeId AS nid_007, score AS pr_score_007
WITH nid_007, pr_score_007
WHERE pr_score_007 > 0.362
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 50 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Uncertainty { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_9', {
  maxIterations: 26,
  dampingFactor: 0.808
}) YIELD nodeId AS nid_009, score AS pr_score_009
WITH nid_009, pr_score_009
WHERE pr_score_009 > 0.457
RETURN nid_009, pr_score_009
ORDER BY pr_score_009 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 27 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Uncertainty { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_12', {
  maxIterations: 42,
  dampingFactor: 0.761
}) YIELD nodeId AS nid_012, score AS pr_score_012
WITH nid_012, pr_score_012
WHERE pr_score_012 > 0.184
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 4 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Uncertainty { priority: 8 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
...
CALL gds.pageRank.stream('uncertainty_graph_13', {
  maxIterations: 48,
  dampingFactor: 0.849
}) YIELD nodeId AS nid_013, score AS pr_score_013
WITH nid_013, pr_score_013
WHERE pr_score_013 > 0.444
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 38 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Uncertainty { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Uncertainty'
