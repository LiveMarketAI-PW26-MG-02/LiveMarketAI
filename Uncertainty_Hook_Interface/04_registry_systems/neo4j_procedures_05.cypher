...
CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 16 RETURN n",
...
WHERE pr_score_000 > 0.124
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 26 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_007 > 0.102
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 44 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
