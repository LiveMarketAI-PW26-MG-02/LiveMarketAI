...
CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 29 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
WHERE pr_score_002 > 0.414
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 39 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
WHERE pr_score_004 > 0.126
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 17 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 5 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
WHERE pr_score_013 > 0.354
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Uncertainty) WHERE n.epoch = 23 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
