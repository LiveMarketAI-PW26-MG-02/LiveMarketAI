# Explainable Insider-Trading Prediction Framework

Surveillance framework that scores trading patterns against public signals and explains each flag for a compliance reviewer.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /trades, /insiders, /filings, /prediction_signals, /risk_scores
- **AI primitive**: `anomaly` — see `explainable_insider_trading_prediction_framework/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn explainable_insider_trading_prediction_framework.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    explainable_insider_trading_prediction_framework/
