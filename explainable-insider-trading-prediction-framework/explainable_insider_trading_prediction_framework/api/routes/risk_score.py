...
from ...schemas.risk_score import RiskScoreCreate, RiskScoreRead
...
@router.get("/{item_id}", response_model=RiskScoreRead)
def get_item(item_id: int, db: Session = Depends(get_db)):
    obj = repo.get(db, item_id)
    if obj is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "RiskScore not found")
    return obj


@router.post("", response_model=RiskScoreRead, status_code=status.HTTP_201_CREATED)
def create_item(payload: RiskScoreCreate, db: Session = Depends(get_db)):
