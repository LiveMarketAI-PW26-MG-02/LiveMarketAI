...
from ...schemas.insider import InsiderCreate, InsiderRead

router = APIRouter(prefix="/insiders", tags=["Insider"])


@router.get("", response_model=list[InsiderRead])
def list_items(limit: int = 100, offset: int = 0, db: Session = Depends(get_db)):
    return repo.list(db, limit=limit, offset=offset)


@router.get("/{item_id}", response_model=InsiderRead)
def get_item(item_id: int, db: Session = Depends(get_db)):
    obj = repo.get(db, item_id)
    if obj is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Insider not found")
    return obj


@router.post("", response_model=InsiderRead, status_code=status.HTTP_201_CREATED)
def create_item(payload: InsiderCreate, db: Session = Depends(get_db)):
