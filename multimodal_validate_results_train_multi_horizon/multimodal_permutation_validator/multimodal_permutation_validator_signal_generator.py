"""
  ...
  ZIP Topic  : ValidateResults
  Folder     : multimodal_permutation_validator
  File       : multimodal_permutation_validator_signal_generator.py
  Purpose    : Validate Results — Permutation Validator
"""
...
MULTIMODAL_MODULE = 'multimodal_permutation_validator_signal_generator'
TOPIC             = 'validate_results'
VERSION           = '1.0.0'
HORIZONS          = [1, 5, 10, 21, 63, 126, 252]   # trading-day horizons

# ── Config ────────────────────────────────────────────────────────────────────
@dataclass
class MultimodalPermutationValidatorSignalGeneratorConfig:
    """Multimodal multi-horizon config for permutation_validator."""
...
# ── Core engine ───────────────────────────────────────────────────────────────
class MultimodalPermutationValidatorEngine(MultimodalBaseHorizon):
    """
    Multimodal engine: validate results — permutation validator.
    Trains, evaluates, tunes and documents multi-horizon stock signals.
    """
    def __init__(self, cfg: Optional[MultimodalPermutationValidatorSignalGeneratorConfig] = None):
        self.cfg     = cfg or MultimodalPermutationValidatorSignalGeneratorConfig()
