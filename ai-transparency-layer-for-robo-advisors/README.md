# AI Transparency Layer for Robo-Advisors

A drop-in layer that explains every robo-advisor allocation decision and produces compliance-ready rationales.

> **Not financial advice.** This system produces explanatory and informational output only and is not a recommendation to buy or sell any security.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /advisors, /allocations, /rationales, /client_profiles, /compliance_checks
- **AI primitive**: `attention` — see `ai_transparency_layer_for_robo_advisors/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn ai_transparency_layer_for_robo_advisors.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    ai_transparency_layer_for_robo_advisors/
