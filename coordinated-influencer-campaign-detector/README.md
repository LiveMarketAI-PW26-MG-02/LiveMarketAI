...
Detects coordinated promotional campaigns across financial influencers using a posting-behaviour graph.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /influencers, /posts, /campaign_signals, /edges, /detection_alerts
- **AI primitive**: `graph` — see `coordinated_influencer_campaign_detector/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn coordinated_influencer_campaign_detector.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    coordinated_influencer_campaign_detector/
