# AI-Driven Retail-Investor Confidence Meter

Aggregates social and market signals into an explainable retail confidence index with source attribution.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /sentiment_signals, /confidence_indexs, /cohorts, /sources, /snapshots
- **AI primitive**: `nlp` — see `ai_driven_retail_investor_confidence_meter/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn ai_driven_retail_investor_confidence_meter.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    ai_driven_retail_investor_confidence_meter/
