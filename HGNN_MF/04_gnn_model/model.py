#!/usr/bin/env python3
"""
Heterogeneous Graph Neural Fusion (HGNN-MF) - GNN Model — model.py Entry
Implements the HeterogeneousGraphFusion graph neural network architecture
...
OUTPUT_DIR = "outputs/hgnn_mf_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in HGNN-MF.
    """
...
class GraphAttentionLayer:
    ...
    Critical for HGNN-MF to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of HGNN-MF: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in HGNN-MF.
    """
...
class HeterogeneousGraphFusion:
    """
    Heterogeneous Graph Neural Fusion (HGNN-MF) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the HGNN-MF GNN.
...
def build_model(config: Optional[dict] = None) -> HeterogeneousGraphFusion:
    """Factory function to build HGNN-MF model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = HeterogeneousGraphFusion(default_config)
    print(f"  Built HGNN-MF model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  HGNN-MF - GNN Model Module")
    ...
    print(f"\nHGNN-MF GNN Model Module Complete.\n")
    return model, output
