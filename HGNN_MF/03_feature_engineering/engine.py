#!/usr/bin/env python3
"""
Heterogeneous Graph Neural Fusion (HGNN-MF) - Feature Engineering Engine
...
OUTPUT_DIR = "outputs/hgnn_mf_features"
...
class FeaturePipelineOrchestrator:
    """Orchestrates the complete feature engineering pipeline for HGNN-MF."""
    ...
    def run(self, price_df: pd.DataFrame,
            ...
            G=None) -> pd.DataFrame:
        print(f"  [HGNN-MF] Running feature pipeline on {len(price_df)} rows...")
        ...
        print(f"  [HGNN-MF] Feature matrix: {enriched.shape}")
        return enriched
...
def main():
    print(f"\nHGNN-MF Feature Engineering Engine")
    ...
    print(f"\nHGNN-MF Feature Engineering complete.\n")
    return feat_df
