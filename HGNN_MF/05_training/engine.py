#!/usr/bin/env python3
"""
Heterogeneous Graph Neural Fusion (HGNN-MF) - Training Engine (Alt Entry)
Implements training loop, loss functions, optimization, and
evaluation metrics for the HGNN-MF manipulation detection model.
"""
...
OUTPUT_DIR = "outputs/hgnn_mf_training"
...
class ManipulationLoss:
    """
    Custom loss function suite for HGNN-MF manipulation detection.
    ...
    def regime_consistency_loss(embeddings: np.ndarray, regime_labels: np.ndarray,
        ...
        Contrastive loss variant for regime-aware HGNN-MF.
        """
        total_loss = 0.0
        n = len(regime_labels)
        count = 0
...
class LearningRateScheduler:
    """Learning rate scheduling strategies for HGNN-MF training."""
...
class TrainingDataGenerator:
    """Generates synthetic training batches for HGNN-MF GNN training loop."""
...
class GNNTrainer:
    """
    Complete training loop for HGNN-MF GNN model.
    ...
    def fit(self, train_data: List[dict], val_data: List[dict],
        ...
        print(f"\n  Training HGNN-MF for {n_epochs} epochs...")
...
def main():
    print(f"\n{'='*60}")
    print(f"  HGNN-MF - Training Engine (Alt Entry)")
    ...
    print(f"\nHGNN-MF Training Engine (Alt Entry) Complete.\n")
