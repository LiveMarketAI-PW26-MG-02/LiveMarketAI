...
from ...schemas.executive import ExecutiveCreate, ExecutiveRead

router = APIRouter(prefix="/executives", tags=["Executive"])
...
@router.get("/{item_id}", response_model=ExecutiveRead)
...
@router.post("", response_model=ExecutiveRead, status_code=status.HTTP_201_CREATED)
def create_item(payload: ExecutiveCreate, db: Session = Depends(get_db)):
