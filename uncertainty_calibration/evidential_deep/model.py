"""
Evidential deep learning for uncertainty separation
Module: evidential_deep.model
"""
import torch
...
class EvidentialDeepRegression(nn.Module):
    """
    Evidential deep learning for uncertainty separation
    """
...
def evidential_loss(gamma, nu, alpha, beta, target, lam=1.0):
    omega = 2 * beta * (1 + nu)
    nll = 0.5*(torch.log(torch.pi/nu) - alpha*torch.log(omega)
               + (alpha+0.5)*torch.log((target-gamma).pow(2)*nu+omega)
               + torch.lgamma(alpha) - torch.lgamma(alpha+0.5))
    reg = (target-gamma).abs() * (2*nu+alpha)
