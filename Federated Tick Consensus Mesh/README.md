# PhantomTick Poisoning Defense
**System ID:** SYS-001
**Abbreviation:** FTCM

## Overview
Federated Tick Consensus Mesh: Independent ingestion nodes train localized statistical microstructure models on tick distribution properties including spread variance, inter-arrival latency entropy, and volatility micro-bursts. Instead of sharing raw financial data, nodes share model gradient updates. A federated aggregation layer computes a global anomaly consensus vector. Each incoming tick stream is evaluated against both local and global models. Deviation beyond dynamic divergence thresholds triggers containment protocols.
...
java -jar target/ftcm-java-node-1.0.0-SNAPSHOT.jar
