"""
  ...
  ZIP Topic  : Summarize
  Folder     : multimodal_table_aggregator
  File       : multimodal_table_aggregator_signal_generator.py
  Purpose    : Summarize — Table Aggregator
"""
...
MULTIMODAL_MODULE = 'multimodal_table_aggregator_signal_generator'
TOPIC             = 'summarize'
VERSION           = '1.0.0'
...
# ── Config ────────────────────────────────────────────────────────────────────
@dataclass
class MultimodalTableAggregatorSignalGeneratorConfig:
    """Multimodal multi-horizon config for table_aggregator."""
...
# ── Core engine ───────────────────────────────────────────────────────────────
class MultimodalTableAggregatorEngine(MultimodalBaseHorizon):
    """
    Multimodal engine: summarize — table aggregator.
    Trains, evaluates, tunes and documents multi-horizon stock signals.
    """
    def __init__(self, cfg: Optional[MultimodalTableAggregatorSignalGeneratorConfig] = None):
        self.cfg     = cfg or MultimodalTableAggregatorSignalGeneratorConfig()
