"""
  ...
  ZIP Topic  : Summarize
  Folder     : multimodal_chart_data_compiler
  File       : multimodal_chart_data_compiler_signal_generator.py
  Purpose    : Summarize — Chart Data Compiler
"""
...
MULTIMODAL_MODULE = 'multimodal_chart_data_compiler_signal_generator'
TOPIC             = 'summarize'
VERSION           = '1.0.0'
...
# ── Config ────────────────────────────────────────────────────────────────────
@dataclass
class MultimodalChartDataCompilerSignalGeneratorConfig:
    """Multimodal multi-horizon config for chart_data_compiler."""
...
# ── Core engine ───────────────────────────────────────────────────────────────
class MultimodalChartDataCompilerEngine(MultimodalBaseHorizon):
    """
    Multimodal engine: summarize — chart data compiler.
    Trains, evaluates, tunes and documents multi-horizon stock signals.
    """
    def __init__(self, cfg: Optional[MultimodalChartDataCompilerSignalGeneratorConfig] = None):
        self.cfg     = cfg or MultimodalChartDataCompilerSignalGeneratorConfig()
