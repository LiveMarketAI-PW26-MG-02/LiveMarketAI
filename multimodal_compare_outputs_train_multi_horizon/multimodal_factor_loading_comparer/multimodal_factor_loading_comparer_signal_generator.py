"""
  ...
  ZIP Topic  : CompareOutputs
  Folder     : multimodal_factor_loading_comparer
  File       : multimodal_factor_loading_comparer_signal_generator.py
  Purpose    : Compare Outputs — Factor Loading Comparer
"""
...
MULTIMODAL_MODULE = 'multimodal_factor_loading_comparer_signal_generator'
TOPIC             = 'compare_outputs'
VERSION           = '1.0.0'
HORIZONS          = [1, 5, 10, 21, 63, 126, 252]   # trading-day horizons

# ── Config ────────────────────────────────────────────────────────────────────
@dataclass
class MultimodalFactorLoadingComparerSignalGeneratorConfig:
    """Multimodal multi-horizon config for factor_loading_comparer."""
...
# ── Core engine ───────────────────────────────────────────────────────────────
class MultimodalFactorLoadingComparerEngine(MultimodalBaseHorizon):
    """
    Multimodal engine: compare outputs — factor loading comparer.
    Trains, evaluates, tunes and documents multi-horizon stock signals.
    """
    def __init__(self, cfg: Optional[MultimodalFactorLoadingComparerSignalGeneratorConfig] = None):
        self.cfg     = cfg or MultimodalFactorLoadingComparerSignalGeneratorConfig()
