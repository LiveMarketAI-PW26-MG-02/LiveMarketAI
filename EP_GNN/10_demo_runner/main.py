#!/usr/bin/env python3
"""
Emotion Propagation Graph Neural Network (EP-GNN) - Demo Runner — End-to-End Pipeline
...
OUTPUT_DIR = "outputs/ep_gnn_inference"
...
class InferenceFeatureExtractor:
    ...
    for EP-GNN inference. Handles normalization using training statistics.
    """
...
class AlertGenerator:
    ...
    def save_alerts(self, alerts: List[dict], filename: str = "alerts.json") -> str:
        ...
        with open(path, "w") as f:
            json.dump({"module": "EP-GNN", "alerts": alerts,
                        "total": len(alerts),
...
class InferencePipeline:
    """
    End-to-end EP-GNN inference pipeline:
    ...
    def __init__(self, model, n_assets: int = 20, threshold: float = 0.5):
        ...
        self.alert_generator = AlertGenerator("EP-GNN")
    ...
    def run_streaming(self, n_windows: int = 50,
        ...
        print(f"  Running EP-GNN streaming inference: {n_windows} windows...")
        results = []
...
def main():
    print(f"\n{'='*60}")
    print(f"  EP-GNN - Demo Runner — End-to-End Pipeline")
    print(f"  Emotion Propagation Graph Neural Network")
    print(f"{'='*60}\n")
    ...
    print(f"\nEP-GNN Demo Runner — End-to-End Pipeline Complete.\n")
...
7 + np.random.rand(n) * 0.3, 0, 1),
        ...
        explanation_text = f"""
MANIPULATION DETECTION EXPLANATION - EP-GNN
============================================================
