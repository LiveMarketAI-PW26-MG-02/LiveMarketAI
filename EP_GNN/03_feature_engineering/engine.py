#!/usr/bin/env python3
"""
Emotion Propagation Graph Neural Network (EP-GNN) - Feature Engineering Engine
...
OUTPUT_DIR = "outputs/ep_gnn_features"
...
class FeaturePipelineOrchestrator:
    """Orchestrates the complete feature engineering pipeline for EP-GNN."""
    ...
    def run(self, price_df: pd.DataFrame,
            ...
            G=None) -> pd.DataFrame:
        print(f"  [EP-GNN] Running feature pipeline on {len(price_df)} rows...")
        ...
        print(f"  [EP-GNN] Feature matrix: {enriched.shape}")
        return enriched
...
def main():
    print(f"\nEP-GNN Feature Engineering Engine")
    ...
    print(f"\nEP-GNN Feature Engineering complete.\n")
    return feat_df
