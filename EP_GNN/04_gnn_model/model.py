#!/usr/bin/env python3
"""
Emotion Propagation Graph Neural Network (EP-GNN) - GNN Model — model.py Entry
Implements the EmotionPropagationGNN graph neural network architecture
...
OUTPUT_DIR = "outputs/ep_gnn_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in EP-GNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for EP-GNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of EP-GNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in EP-GNN.
    """
...
class EmotionPropagationGNN:
    """
    Emotion Propagation Graph Neural Network (EP-GNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the EP-GNN GNN.
...
def build_model(config: Optional[dict] = None) -> EmotionPropagationGNN:
    """Factory function to build EP-GNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = EmotionPropagationGNN(default_config)
    print(f"  Built EP-GNN model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  EP-GNN - GNN Model Module")
    ...
    print(f"\nEP-GNN GNN Model Module Complete.\n")
    return model, output
