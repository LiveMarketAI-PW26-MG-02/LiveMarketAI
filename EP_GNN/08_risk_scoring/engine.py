#!/usr/bin/env python3
"""
Emotion Propagation Graph Neural Network (EP-GNN) - Risk Scoring Engine (Alt Entry)
Computes emotion_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/ep_gnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for EP-GNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: EP-GNN | Primary risk metric: emotion_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["emotion_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["ep_gnn_severity"] = pd.cut(
            result["emotion_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["ep_gnn_label"] = (result["emotion_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for EP-GNN."""
...
def main():
    ...
    print(f"  Mean {'emotion_manipulation_score'}: {scored_df['emotion_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['emotion_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'emotion_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["emotion_manipulation_score"], "severity": r["ep_gnn_severity"], "module": "EP-GNN"}
        for i, r in scored_df[scored_df["ep_gnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"EP-GNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nEP-GNN Risk Scoring Complete.\n")
