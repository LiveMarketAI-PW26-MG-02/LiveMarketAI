#!/usr/bin/env python3
"""
Regime-Aware Graph Neural Network (RAGNN) - Risk Scoring Engine (Alt Entry)
Computes regime_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/ragnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for RAGNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: RAGNN | Primary risk metric: regime_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["regime_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["ragnn_severity"] = pd.cut(
            result["regime_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["ragnn_label"] = (result["regime_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for RAGNN."""
...
def main():
    ...
    print(f"  Mean {'regime_manipulation_score'}: {scored_df['regime_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['regime_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'regime_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["regime_manipulation_score"], "severity": r["ragnn_severity"], "module": "RAGNN"}
        for i, r in scored_df[scored_df["ragnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"RAGNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nRAGNN Risk Scoring Complete.\n")
