#!/usr/bin/env python3
"""
Regime-Aware Graph Neural Network (RAGNN) - Demo Runner — End-to-End Pipeline
...
OUTPUT_DIR = "outputs/ragnn_inference"
...
class InferenceFeatureExtractor:
    ...
    for RAGNN inference. Handles normalization using training statistics.
    """
...
class AlertGenerator:
    ...
    def save_alerts(self, alerts: List[dict], filename: str = "alerts.json") -> str:
        ...
        with open(path, "w") as f:
            json.dump({"module": "RAGNN", "alerts": alerts,
                        "total": len(alerts),
...
class InferencePipeline:
    """
    End-to-end RAGNN inference pipeline:
    ...
    def __init__(self, model, n_assets: int = 20, threshold: float = 0.5):
        ...
        self.alert_generator = AlertGenerator("RAGNN")
    ...
    def run_streaming(self, n_windows: int = 50,
        ...
        print(f"  Running RAGNN streaming inference: {n_windows} windows...")
        results = []
...
def main():
    print(f"\n{'='*60}")
    print(f"  RAGNN - Demo Runner — End-to-End Pipeline")
    ...
    print(f"\nRAGNN Demo Runner — End-to-End Pipeline Complete.\n")
...
7 + np.random.rand(n) * 0.3, 0, 1),
        ...
        explanation_text = f"""
MANIPULATION DETECTION EXPLANATION - RAGNN
============================================================
