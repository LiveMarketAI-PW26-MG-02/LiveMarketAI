#!/usr/bin/env python3
"""
Regime-Aware Graph Neural Network (RAGNN) - Training Engine (Alt Entry)
Implements training loop, loss functions, optimization, and
evaluation metrics for the RAGNN manipulation detection model.
"""
...
OUTPUT_DIR = "outputs/ragnn_training"
...
class ManipulationLoss:
    """
    Custom loss function suite for RAGNN manipulation detection.
    ...
    def regime_consistency_loss(embeddings: np.ndarray, regime_labels: np.ndarray,
        ...
        Contrastive loss variant for regime-aware RAGNN.
        """
        total_loss = 0.0
        n = len(regime_labels)
        count = 0
...
class LearningRateScheduler:
    """Learning rate scheduling strategies for RAGNN training."""
...
class TrainingDataGenerator:
    """Generates synthetic training batches for RAGNN GNN training loop."""
...
class GNNTrainer:
    """
    Complete training loop for RAGNN GNN model.
    ...
    def fit(self, train_data: List[dict], val_data: List[dict],
        ...
        print(f"\n  Training RAGNN for {n_epochs} epochs...")
...
def main():
    print(f"\n{'='*60}")
    print(f"  RAGNN - Training Engine (Alt Entry)")
    ...
    print(f"\nRAGNN Training Engine (Alt Entry) Complete.\n")
