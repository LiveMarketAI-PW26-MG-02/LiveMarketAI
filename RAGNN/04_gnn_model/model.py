#!/usr/bin/env python3
"""
Regime-Aware Graph Neural Network (RAGNN) - GNN Model — model.py Entry
Implements the RegimeAwareGNN graph neural network architecture
...
OUTPUT_DIR = "outputs/ragnn_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in RAGNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for RAGNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of RAGNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in RAGNN.
    """
...
class RegimeAwareGNN:
    """
    Regime-Aware Graph Neural Network (RAGNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the RAGNN GNN.
...
def build_model(config: Optional[dict] = None) -> RegimeAwareGNN:
    """Factory function to build RAGNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = RegimeAwareGNN(default_config)
    print(f"  Built RAGNN model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  RAGNN - GNN Model Module")
    ...
    print(f"\nRAGNN GNN Model Module Complete.\n")
    return model, output
