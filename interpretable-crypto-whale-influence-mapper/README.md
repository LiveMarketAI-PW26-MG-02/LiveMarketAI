...
Maps on-chain influence between large wallets and explains how price moves propagate through the holder graph.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /wallets, /transactions, /whale_clusters, /influence_edges, /tokens
- **AI primitive**: `graph` — see `interpretable_crypto_whale_influence_mapper/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn interpretable_crypto_whale_influence_mapper.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    interpretable_crypto_whale_influence_mapper/
