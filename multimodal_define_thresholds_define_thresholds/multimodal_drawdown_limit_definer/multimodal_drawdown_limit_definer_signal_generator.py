"""
  ...
  ZIP Topic  : DefineThresholds
  Folder     : multimodal_drawdown_limit_definer
  File       : multimodal_drawdown_limit_definer_signal_generator.py
  Purpose    : Define Thresholds — Drawdown Limit Definer
"""
...
MULTIMODAL_MODULE = 'multimodal_drawdown_limit_definer_signal_generator'
TOPIC             = 'define_thresholds'
VERSION           = '1.0.0'
DEFAULT_THRESHOLDS = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0]

# ── Configuration ─────────────────────────────────────────────────────────────
@dataclass
class MultimodalDrawdownLimitDefinerSignalGeneratorConfig:
    """Multimodal threshold-sweep config for drawdown_limit_definer."""
...
# ── Core Engine ───────────────────────────────────────────────────────────────
class MultimodalDrawdownLimitDefinerEngine(MultimodalThresholdBase):
    """
    Multimodal threshold engine: define thresholds — drawdown limit definer.
    Sweeps, evaluates, validates and documents signal thresholds.
    """
    def __init__(self, cfg: Optional[MultimodalDrawdownLimitDefinerSignalGeneratorConfig] = None):
        self.cfg      = cfg or MultimodalDrawdownLimitDefinerSignalGeneratorConfig()
