...
from explainable_derivatives_risk_decomposition_engine.main import app


def test_create_and_list_derivative():
    with TestClient(app) as c:
        created = c.post("/derivatives", json={})
        assert created.status_code == 201
        listed = c.get("/derivatives")
