:param namespace => 'exceptionrecovery_05_05';
:param batchSize => 512;
:param threshold => 0.264;
:param maxDepth => 8;
...
CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 31 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:ExceptionRecovery { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_0', {
  maxIterations: 22,
  dampingFactor: 0.739
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.391
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 17 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:ExceptionRecovery { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_1', {
  maxIterations: 37,
  dampingFactor: 0.787
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.491
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 24 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:ExceptionRecovery { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_2', {
  maxIterations: 32,
  dampingFactor: 0.738
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.12
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 14 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_3', {
  maxIterations: 41,
  dampingFactor: 0.729
}) YIELD nodeId AS nid_003, score AS pr_score_003
WITH nid_003, pr_score_003
WHERE pr_score_003 > 0.153
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 22 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:ExceptionRecovery { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_4', {
  maxIterations: 27,
  dampingFactor: 0.716
}) YIELD nodeId AS nid_004, score AS pr_score_004
WITH nid_004, pr_score_004
WHERE pr_score_004 > 0.227
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 15 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:ExceptionRecovery { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_5', {
  maxIterations: 12,
  dampingFactor: 0.782
}) YIELD nodeId AS nid_005, score AS pr_score_005
WITH nid_005, pr_score_005
WHERE pr_score_005 > 0.203
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 4 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_6', {
  maxIterations: 18,
  dampingFactor: 0.867
...
CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 37 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
CALL gds.pageRank.stream('exceptionrecovery_graph_7', {
  maxIterations: 45,
  dampingFactor: 0.759
}) YIELD nodeId AS nid_007, score AS pr_score_007
WITH nid_007, pr_score_007
WHERE pr_score_007 > 0.462
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 45 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);

MATCH (start:ExceptionRecovery { priority: 2 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_8', {
  maxIterations: 50,
  dampingFactor: 0.727
}) YIELD nodeId AS nid_008, score AS pr_score_008
WITH nid_008, pr_score_008
WHERE pr_score_008 > 0.351
RETURN nid_008, pr_score_008
ORDER BY pr_score_008 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 28 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_9', {
  maxIterations: 50,
  dampingFactor: 0.883
}) YIELD nodeId AS nid_009, score AS pr_score_009
WITH nid_009, pr_score_009
WHERE pr_score_009 > 0.164
RETURN nid_009, pr_score_009
ORDER BY pr_score_009 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 42 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:ExceptionRecovery { priority: 8 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_10', {
  maxIterations: 49,
  dampingFactor: 0.816
}) YIELD nodeId AS nid_010, score AS pr_score_010
WITH nid_010, pr_score_010
WHERE pr_score_010 > 0.329
RETURN nid_010, pr_score_010
ORDER BY pr_score_010 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 3 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);

MATCH (start:ExceptionRecovery { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+ExceptionRecovery'
...
CALL gds.pageRank.stream('exceptionrecovery_graph_11', {
  maxIterations: 39,
  dampingFactor: 0.921
}) YIELD nodeId AS nid_011, score AS pr_score_011
WITH nid_011, pr_score_011
WHERE pr_score_011 > 0.182
RETURN nid_011, pr_score_011
ORDER BY pr_score_011 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 38 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_12', {
  maxIterations: 47,
  dampingFactor: 0.842
}) YIELD nodeId AS nid_012, score AS pr_score_012
WITH nid_012, pr_score_012
WHERE pr_score_012 > 0.487
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 11 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_13', {
  maxIterations: 30,
  dampingFactor: 0.906
}) YIELD nodeId AS nid_013, score AS pr_score_013
WITH nid_013, pr_score_013
WHERE pr_score_013 > 0.155
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 35 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_14', {
  maxIterations: 18,
  dampingFactor: 0.806
}) YIELD nodeId AS nid_014, score AS pr_score_014
WITH nid_014, pr_score_014
WHERE pr_score_014 > 0.244
RETURN nid_014, pr_score_014
ORDER BY pr_score_014 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 21 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_15', {
  maxIterations: 32,
  dampingFactor: 0.724
}) YIELD nodeId AS nid_015, score AS pr_score_015
WITH nid_015, pr_score_015
WHERE pr_score_015 > 0.494
RETURN nid_015, pr_score_015
ORDER BY pr_score_015 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 36 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_16', {
  maxIterations: 19,
  dampingFactor: 0.855
}) YIELD nodeId AS nid_016, score AS pr_score_016
WITH nid_016, pr_score_016
WHERE pr_score_016 > 0.404
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:ExceptionRecovery) WHERE n.epoch = 40 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('exceptionrecovery_graph_17', {
  maxIterations: 17,
  dampingFactor: 0.746
}) YIELD nodeId AS nid_017, score AS pr_score_017
WITH nid_017, pr_score_017
WHERE pr_score_017 > 0.112
