# Coordinated Whale-Manipulation Detector

Surfaces coordinated activity between large holders using graph centrality and timing correlation.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /wallets, /coordination_signals, /whale_groups, /trades, /edges
- **AI primitive**: `graph` — see `coordinated_whale_manipulation_detector/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn coordinated_whale_manipulation_detector.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    coordinated_whale_manipulation_detector/
