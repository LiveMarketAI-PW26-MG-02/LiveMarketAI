#!/usr/bin/env python3
"""
Multi-Scale Graph Neural Network (MSGNN) - GNN Model — model.py Entry
Implements the MultiScaleGNN graph neural network architecture
...
OUTPUT_DIR = "outputs/msgnn_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in MSGNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for MSGNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of MSGNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in MSGNN.
    """
...
class MultiScaleGNN:
    """
    Multi-Scale Graph Neural Network (MSGNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the MSGNN GNN.
...
def build_model(config: Optional[dict] = None) -> MultiScaleGNN:
    """Factory function to build MSGNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = MultiScaleGNN(default_config)
    print(f"  Built MSGNN model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  MSGNN - GNN Model Module")
    ...
    print(f"\nMSGNN GNN Model Module Complete.\n")
    return model, output
