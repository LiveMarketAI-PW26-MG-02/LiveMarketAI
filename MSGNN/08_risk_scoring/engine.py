#!/usr/bin/env python3
"""
Multi-Scale Graph Neural Network (MSGNN) - Risk Scoring Engine (Alt Entry)
Computes multiscale_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/msgnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for MSGNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: MSGNN | Primary risk metric: multiscale_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["multiscale_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["msgnn_severity"] = pd.cut(
            result["multiscale_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["msgnn_label"] = (result["multiscale_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for MSGNN."""
...
def main():
    ...
    print(f"  Mean {'multiscale_manipulation_score'}: {scored_df['multiscale_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['multiscale_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'multiscale_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["multiscale_manipulation_score"], "severity": r["msgnn_severity"], "module": "MSGNN"}
        for i, r in scored_df[scored_df["msgnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"MSGNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nMSGNN Risk Scoring Complete.\n")
