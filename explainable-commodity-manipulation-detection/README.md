...
Detects and explains manipulative patterns in commodity markets such as wash trading, ramping and benchmark fixing.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /commodity_ticks, /manipulation_alerts, /patterns, /venues, /explanations
- **AI primitive**: `anomaly` — see `explainable_commodity_manipulation_detection/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn explainable_commodity_manipulation_detection.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    explainable_commodity_manipulation_detection/
