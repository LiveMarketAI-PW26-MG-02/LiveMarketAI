# Cross-Exchange Spoofing-Detection AI

Correlates order books across venues to detect cross-exchange spoofing and layering, with attributable explanations.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /order_book_snapshots, /spoof_alerts, /exchanges, /order_events, /explanations
- **AI primitive**: `anomaly` — see `cross_exchange_spoofing_detection_ai/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn cross_exchange_spoofing_detection_ai.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    cross_exchange_spoofing_detection_ai/
