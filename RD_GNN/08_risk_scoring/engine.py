#!/usr/bin/env python3
"""
Risk Diffusion Graph Network (RD-GNN) - Risk Scoring Engine (Alt Entry)
Computes systemic_risk_diffusion_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/rd_gnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for RD-GNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: RD-GNN | Primary risk metric: systemic_risk_diffusion_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["systemic_risk_diffusion_score"] = np.clip(ensemble, 0, 1)
        result["rd_gnn_severity"] = pd.cut(
            result["systemic_risk_diffusion_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["rd_gnn_label"] = (result["systemic_risk_diffusion_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for RD-GNN."""
...
def main():
    ...
    print(f"  Mean {'systemic_risk_diffusion_score'}: {scored_df['systemic_risk_diffusion_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['systemic_risk_diffusion_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'systemic_risk_diffusion_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["systemic_risk_diffusion_score"], "severity": r["rd_gnn_severity"], "module": "RD-GNN"}
        for i, r in scored_df[scored_df["rd_gnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"RD-GNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nRD-GNN Risk Scoring Complete.\n")
