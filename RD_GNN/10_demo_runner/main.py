#!/usr/bin/env python3
"""
Risk Diffusion Graph Network (RD-GNN) - Demo Runner — End-to-End Pipeline
...
OUTPUT_DIR = "outputs/rd_gnn_inference"
...
class InferenceFeatureExtractor:
    ...
    for RD-GNN inference. Handles normalization using training statistics.
    """
...
class AlertGenerator:
    ...
    def __init__(self, module_name: str = "RD-GNN"):
    ...
    def save_alerts(self, alerts: List[dict], filename: str = "alerts.json") -> str:
        ...
        with open(path, "w") as f:
            json.dump({"module": "RD-GNN", "alerts": alerts,
                        "total": len(alerts),
...
class InferencePipeline:
    """
    End-to-end RD-GNN inference pipeline:
    ...
    def __init__(self, model, n_assets: int = 20, threshold: float = 0.5):
        ...
        self.alert_generator = AlertGenerator("RD-GNN")
    ...
    def run_streaming(self, n_windows: int = 50,
        ...
        print(f"  Running RD-GNN streaming inference: {n_windows} windows...")
        results = []
...
def main():
    print(f"\n{'='*60}")
    print(f"  RD-GNN - Demo Runner — End-to-End Pipeline")
    print(f"  Risk Diffusion Graph Network")
    print(f"{'='*60}\n")
    ...
    print(f"\nRD-GNN Demo Runner — End-to-End Pipeline Complete.\n")
...
7 + np.random.rand(n) * 0.3, 0, 1),
        ...
        explanation_text = f"""
MANIPULATION DETECTION EXPLANATION - RD-GNN
============================================================
...
"""
        ...
        report  = {
            "module":          "RD-GNN",
            "full_name":       "Risk Diffusion Graph Network",
