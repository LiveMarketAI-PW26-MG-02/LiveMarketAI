#!/usr/bin/env python3
"""
Risk Diffusion Graph Network (RD-GNN) - GNN Model — model.py Entry
Implements the RiskDiffusionGNN graph neural network architecture
...
OUTPUT_DIR = "outputs/rd_gnn_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in RD-GNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for RD-GNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of RD-GNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in RD-GNN.
    """
...
class RiskDiffusionGNN:
    """
    Risk Diffusion Graph Network (RD-GNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the RD-GNN GNN.
...
def build_model(config: Optional[dict] = None) -> RiskDiffusionGNN:
    """Factory function to build RD-GNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = RiskDiffusionGNN(default_config)
    print(f"  Built RD-GNN model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  RD-GNN - GNN Model Module")
    ...
    print(f"\nRD-GNN GNN Model Module Complete.\n")
    return model, output
