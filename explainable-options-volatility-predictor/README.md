# Explainable Options-Volatility Predictor

Predicts implied-volatility surface moves and explains each forecast with per-feature contributions.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /options, /vol_surfaces, /predictions, /feature_contributions, /underlyings
- **AI primitive**: `forecast` — see `explainable_options_volatility_predictor/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn explainable_options_volatility_predictor.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    explainable_options_volatility_predictor/
