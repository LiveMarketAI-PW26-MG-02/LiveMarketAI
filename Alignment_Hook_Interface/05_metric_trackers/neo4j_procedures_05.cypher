:param namespace => 'alignment_05_05';
:param batchSize => 64;
:param threshold => 0.835;
:param maxDepth => 8;
...
CALL gds.pageRank.stream('alignment_graph_1', {
  maxIterations: 25,
  dampingFactor: 0.945
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.113
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 25 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: false}
);

MATCH (start:Alignment { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_2', {
  maxIterations: 43,
  dampingFactor: 0.813
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.382
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 43 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:Alignment { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_4', {
  maxIterations: 25,
  dampingFactor: 0.857
}) YIELD nodeId AS nid_004, score AS pr_score_004
WITH nid_004, pr_score_004
WHERE pr_score_004 > 0.152
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 30 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Alignment { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_5', {
  maxIterations: 38,
  dampingFactor: 0.782
...
CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 36 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
...
CALL gds.pageRank.stream('alignment_graph_6', {
  maxIterations: 49,
  dampingFactor: 0.94
...
CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 16 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Alignment { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_8', {
  maxIterations: 22,
  dampingFactor: 0.828
}) YIELD nodeId AS nid_008, score AS pr_score_008
WITH nid_008, pr_score_008
WHERE pr_score_008 > 0.264
RETURN nid_008, pr_score_008
ORDER BY pr_score_008 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 9 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:Alignment { priority: 8 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_9', {
  maxIterations: 32,
  dampingFactor: 0.785
}) YIELD nodeId AS nid_009, score AS pr_score_009
WITH nid_009, pr_score_009
WHERE pr_score_009 > 0.178
RETURN nid_009, pr_score_009
ORDER BY pr_score_009 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 23 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:Alignment { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_11', {
  maxIterations: 29,
  dampingFactor: 0.706
...
CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 50 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL gds.pageRank.stream('alignment_graph_12', {
  maxIterations: 14,
  dampingFactor: 0.877
}) YIELD nodeId AS nid_012, score AS pr_score_012
WITH nid_012, pr_score_012
WHERE pr_score_012 > 0.155
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 4 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:Alignment { priority: 5 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+Alignment'
...
CALL gds.pageRank.stream('alignment_graph_13', {
  maxIterations: 35,
  dampingFactor: 0.747
...
CALL apoc.periodic.iterate(
  "MATCH (n:Alignment) WHERE n.epoch = 34 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);
