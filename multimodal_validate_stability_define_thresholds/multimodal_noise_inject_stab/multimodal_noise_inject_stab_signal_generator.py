"""
  ...
  Folder     : multimodal_noise_inject_stab
  File       : multimodal_noise_inject_stab_signal_generator.py
  Purpose    : Validate Stability — Noise Inject Stab
"""
...
MULTIMODAL_MODULE = 'multimodal_noise_inject_stab_signal_generator'
...
# ── Configuration ─────────────────────────────────────────────────────────────
@dataclass
class MultimodalNoiseInjectStabSignalGeneratorConfig:
    """Multimodal threshold-sweep config for noise_inject_stab."""
...
# ── Core Engine ───────────────────────────────────────────────────────────────
class MultimodalNoiseInjectStabEngine(MultimodalThresholdBase):
    """
    Multimodal threshold engine: validate stability — noise inject stab.
    Sweeps, evaluates, validates and documents signal thresholds.
    """
    def __init__(self, cfg: Optional[MultimodalNoiseInjectStabSignalGeneratorConfig] = None):
        self.cfg      = cfg or MultimodalNoiseInjectStabSignalGeneratorConfig()
