...
WHERE pr_score_001 > 0.487
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:CheckpointLoader) WHERE n.epoch = 2 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_003 > 0.121
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:CheckpointLoader) WHERE n.epoch = 6 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_004 > 0.419
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:CheckpointLoader) WHERE n.epoch = 7 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
WHERE pr_score_011 > 0.183
RETURN nid_011, pr_score_011
ORDER BY pr_score_011 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:CheckpointLoader) WHERE n.epoch = 8 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);
