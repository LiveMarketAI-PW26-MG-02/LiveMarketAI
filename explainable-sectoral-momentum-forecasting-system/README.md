...
Forecasts sector momentum and explains each forecast with ranked constituent and macro contributions.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /sectors, /momentum_scores, /forecasts, /feature_contributions, /constituents
- **AI primitive**: `forecast` — see `explainable_sectoral_momentum_forecasting_system/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn explainable_sectoral_momentum_forecasting_system.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    explainable_sectoral_momentum_forecasting_system/
