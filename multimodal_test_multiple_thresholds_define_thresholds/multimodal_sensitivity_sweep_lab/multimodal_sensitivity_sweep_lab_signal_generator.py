"""
  ...
  ZIP Topic  : TestMultipleThresholds
  Folder     : multimodal_sensitivity_sweep_lab
  File       : multimodal_sensitivity_sweep_lab_signal_generator.py
  Purpose    : Test Multiple Thresholds — Sensitivity Sweep Lab
"""
...
MULTIMODAL_MODULE = 'multimodal_sensitivity_sweep_lab_signal_generator'
TOPIC             = 'test_multiple_thresholds'
VERSION           = '1.0.0'
DEFAULT_THRESHOLDS = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0]

# ── Configuration ─────────────────────────────────────────────────────────────
@dataclass
class MultimodalSensitivitySweepLabSignalGeneratorConfig:
    """Multimodal threshold-sweep config for sensitivity_sweep_lab."""
...
# ── Core Engine ───────────────────────────────────────────────────────────────
class MultimodalSensitivitySweepLabEngine(MultimodalThresholdBase):
    """
    Multimodal threshold engine: test multiple thresholds — sensitivity sweep lab.
    Sweeps, evaluates, validates and documents signal thresholds.
    """
    def __init__(self, cfg: Optional[MultimodalSensitivitySweepLabSignalGeneratorConfig] = None):
        self.cfg      = cfg or MultimodalSensitivitySweepLabSignalGeneratorConfig()
