"""
  ...
  ZIP Topic  : TuneParameters
  Folder     : multimodal_regularisation_sweep
  File       : multimodal_regularisation_sweep_signal_generator.py
  Purpose    : Tune Parameters — Regularisation Sweep
"""
...
MULTIMODAL_MODULE = 'multimodal_regularisation_sweep_signal_generator'
TOPIC             = 'tune_parameters'
VERSION           = '1.0.0'
HORIZONS          = [1, 5, 10, 21, 63, 126, 252]   # trading-day horizons

# ── Config ────────────────────────────────────────────────────────────────────
@dataclass
class MultimodalRegularisationSweepSignalGeneratorConfig:
    """Multimodal multi-horizon config for regularisation_sweep."""
...
# ── Core engine ───────────────────────────────────────────────────────────────
class MultimodalRegularisationSweepEngine(MultimodalBaseHorizon):
    """
    Multimodal engine: tune parameters — regularisation sweep.
    Trains, evaluates, tunes and documents multi-horizon stock signals.
    """
    def __init__(self, cfg: Optional[MultimodalRegularisationSweepSignalGeneratorConfig] = None):
        self.cfg     = cfg or MultimodalRegularisationSweepSignalGeneratorConfig()
