#!/usr/bin/env python3
"""
Dynamic Temporal Graph Neural Network (D-TGNN) - Feature Engineering Engine
...
OUTPUT_DIR = "outputs/d_tgnn_features"
...
class FeaturePipelineOrchestrator:
    """Orchestrates the complete feature engineering pipeline for D-TGNN."""
    ...
    def run(self, price_df: pd.DataFrame,
            ...
            G=None) -> pd.DataFrame:
        print(f"  [D-TGNN] Running feature pipeline on {len(price_df)} rows...")
        ...
        print(f"  [D-TGNN] Feature matrix: {enriched.shape}")
        return enriched
...
def main():
    print(f"\nD-TGNN Feature Engineering Engine")
    ...
    print(f"\nD-TGNN Feature Engineering complete.\n")
    return feat_df
