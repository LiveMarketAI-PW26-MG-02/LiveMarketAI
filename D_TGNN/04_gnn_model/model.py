#!/usr/bin/env python3
"""
Dynamic Temporal Graph Neural Network (D-TGNN) - GNN Model — model.py Entry
Implements the DynamicTemporalGNN graph neural network architecture
...
OUTPUT_DIR = "outputs/d_tgnn_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in D-TGNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for D-TGNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of D-TGNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in D-TGNN.
    """
...
class DynamicTemporalGNN:
    """
    Dynamic Temporal Graph Neural Network (D-TGNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the D-TGNN GNN.
...
def build_model(config: Optional[dict] = None) -> DynamicTemporalGNN:
    """Factory function to build D-TGNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = DynamicTemporalGNN(default_config)
    print(f"  Built D-TGNN model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  D-TGNN - GNN Model Module")
    ...
    print(f"\nD-TGNN GNN Model Module Complete.\n")
    return model, output
