#!/usr/bin/env python3
"""
Dynamic Temporal Graph Neural Network (D-TGNN) - Risk Scoring Engine (Alt Entry)
Computes temporal_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/d_tgnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for D-TGNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: D-TGNN | Primary risk metric: temporal_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["temporal_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["d_tgnn_severity"] = pd.cut(
            result["temporal_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["d_tgnn_label"] = (result["temporal_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for D-TGNN."""
...
def main():
    ...
    print(f"  Mean {'temporal_manipulation_score'}: {scored_df['temporal_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['temporal_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'temporal_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["temporal_manipulation_score"], "severity": r["d_tgnn_severity"], "module": "D-TGNN"}
        for i, r in scored_df[scored_df["d_tgnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"D-TGNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nD-TGNN Risk Scoring Complete.\n")
