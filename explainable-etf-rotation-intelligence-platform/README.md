# Explainable ETF-Rotation Intelligence Platform

Detects sector-rotation regimes and explains recommended ETF allocation shifts.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /e_t_fs, /sectors, /rotation_signals, /allocations, /explanations
- **AI primitive**: `forecast` — see `explainable_etf_rotation_intelligence_platform/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn explainable_etf_rotation_intelligence_platform.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    explainable_etf_rotation_intelligence_platform/
