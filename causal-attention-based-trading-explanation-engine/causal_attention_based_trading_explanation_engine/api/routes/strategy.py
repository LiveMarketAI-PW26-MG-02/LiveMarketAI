...
from ...schemas.strategy import StrategyCreate, StrategyRead

router = APIRouter(prefix="/strategys", tags=["Strategy"])
...
@router.get("/{item_id}", response_model=StrategyRead)
...
def create_item(payload: StrategyCreate, db: Session = Depends(get_db)):
