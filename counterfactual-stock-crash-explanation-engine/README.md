# Counterfactual Stock-Crash Explanation Engine

Generates minimal counterfactual scenarios that would have averted a market crash, explaining which factors mattered and by how much.
...
- **10 self-created APIs (routers)**: /health, /auth, /audit, /ingest, /explain, /crash_events, /market_snapshots, /scenarios, /counterfactuals, /explanation_reports
- **AI primitive**: `counterfactual` — see `counterfactual_stock_crash_explanation_engine/ai/engine.py`. Exposed at `POST /explain`.
...
## Run
    ...
    uvicorn counterfactual_stock_crash_explanation_engine.main:app --reload     # SQLite tables auto-created on boot
...
## Layout
    ...
    counterfactual_stock_crash_explanation_engine/
