#!/usr/bin/env python3
"""
Uncertainty-Aware Graph Neural Network (UGNN) - Risk Scoring Engine (Alt Entry)
Computes uncertainty_weighted_risk_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/ugnn_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for UGNN manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: UGNN | Primary risk metric: uncertainty_weighted_risk_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["uncertainty_weighted_risk_score"] = np.clip(ensemble, 0, 1)
        result["ugnn_severity"] = pd.cut(
            result["uncertainty_weighted_risk_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["ugnn_label"] = (result["uncertainty_weighted_risk_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for UGNN."""
...
def main():
    ...
    print(f"  Mean {'uncertainty_weighted_risk_score'}: {scored_df['uncertainty_weighted_risk_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['uncertainty_weighted_risk_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'uncertainty_weighted_risk_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["uncertainty_weighted_risk_score"], "severity": r["ugnn_severity"], "module": "UGNN"}
        for i, r in scored_df[scored_df["ugnn_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"UGNN","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nUGNN Risk Scoring Complete.\n")
