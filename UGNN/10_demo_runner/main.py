#!/usr/bin/env python3
"""
Uncertainty-Aware Graph Neural Network (UGNN) - Demo Runner — End-to-End Pipeline
...
OUTPUT_DIR = "outputs/ugnn_inference"
...
class InferenceFeatureExtractor:
    ...
    for UGNN inference. Handles normalization using training statistics.
    """
...
class AlertGenerator:
    ...
    def __init__(self, module_name: str = "UGNN"):
    ...
    def save_alerts(self, alerts: List[dict], filename: str = "alerts.json") -> str:
        ...
        with open(path, "w") as f:
            json.dump({"module": "UGNN", "alerts": alerts,
                        "total": len(alerts),
...
class InferencePipeline:
    """
    End-to-end UGNN inference pipeline:
    ...
    def __init__(self, model, n_assets: int = 20, threshold: float = 0.5):
        ...
        self.alert_generator = AlertGenerator("UGNN")
    ...
    def run_streaming(self, n_windows: int = 50,
        ...
        print(f"  Running UGNN streaming inference: {n_windows} windows...")
        results = []
...
def main():
    print(f"\n{'='*60}")
    print(f"  UGNN - Demo Runner — End-to-End Pipeline")
    print(f"  Uncertainty-Aware Graph Neural Network")
    print(f"{'='*60}\n")
    ...
    print(f"\nUGNN Demo Runner — End-to-End Pipeline Complete.\n")
...
7 + np.random.rand(n) * 0.3, 0, 1),
        ...
        explanation_text = f"""
MANIPULATION DETECTION EXPLANATION - UGNN
============================================================
...
"""
        ...
        report  = {
            "module":          "UGNN",
            "full_name":       "Uncertainty-Aware Graph Neural Network",
