#!/usr/bin/env python3
"""
Uncertainty-Aware Graph Neural Network (UGNN) - GNN Model — model.py Entry
Implements the UncertaintyAwareGNN graph neural network architecture
...
class GraphConvLayer:
    ...
    Used as a base building block in UGNN.
    """
...
class GraphAttentionLayer:
    ...
    Critical for UGNN to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of UGNN: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in UGNN.
    """
...
class UncertaintyAwareGNN:
    """
    Uncertainty-Aware Graph Neural Network (UGNN) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the UGNN GNN.
...
def build_model(config: Optional[dict] = None) -> UncertaintyAwareGNN:
    """Factory function to build UGNN model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = UncertaintyAwareGNN(default_config)
    print(f"  Built UGNN model with config: {default_config}")
    return model
...
def main():
    ...
    print(f"\nUGNN GNN Model Module Complete.\n")
    return model, output
