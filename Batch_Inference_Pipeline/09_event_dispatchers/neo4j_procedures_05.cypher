...
CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 5 RETURN n",
...
WHERE pr_score_000 > 0.472
RETURN nid_000, pr_score_000
ORDER BY pr_score_000 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 10 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_001 > 0.23
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 20 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_002 > 0.366
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 24 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_003 > 0.499
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 8 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
WHERE pr_score_004 > 0.217
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 19 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
WHERE pr_score_005 > 0.359
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 49 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:BatchInference { priority: 5 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
WHERE pr_score_007 > 0.11
RETURN nid_007, pr_score_007
ORDER BY pr_score_007 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 22 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
...
CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 32 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_012 > 0.346
RETURN nid_012, pr_score_012
ORDER BY pr_score_012 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 40 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
WHERE pr_score_013 > 0.409
RETURN nid_013, pr_score_013
ORDER BY pr_score_013 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 37 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);
...
CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 21 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);
...
WHERE pr_score_016 > 0.376
RETURN nid_016, pr_score_016
ORDER BY pr_score_016 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 15 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
