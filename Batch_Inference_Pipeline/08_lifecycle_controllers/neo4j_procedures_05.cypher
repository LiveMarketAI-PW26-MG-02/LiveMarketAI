:param namespace => 'batchinference_05_05';
:param batchSize => 512;
...
CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 41 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:BatchInference { priority: 9 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_0', {
  maxIterations: 43,
  dampingFactor: 0.866
}) YIELD nodeId AS nid_000, score AS pr_score_000
WITH nid_000, pr_score_000
WHERE pr_score_000 > 0.197
...
CALL gds.pageRank.stream('batchinference_graph_1', {
  maxIterations: 15,
  dampingFactor: 0.878
}) YIELD nodeId AS nid_001, score AS pr_score_001
WITH nid_001, pr_score_001
WHERE pr_score_001 > 0.124
RETURN nid_001, pr_score_001
ORDER BY pr_score_001 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 27 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:BatchInference { priority: 6 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 4,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_2', {
  maxIterations: 47,
  dampingFactor: 0.905
}) YIELD nodeId AS nid_002, score AS pr_score_002
WITH nid_002, pr_score_002
WHERE pr_score_002 > 0.175
RETURN nid_002, pr_score_002
ORDER BY pr_score_002 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 6 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 1000, parallel: true}
);

MATCH (start:BatchInference { priority: 10 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 5,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_3', {
  maxIterations: 35,
  dampingFactor: 0.8
}) YIELD nodeId AS nid_003, score AS pr_score_003
WITH nid_003, pr_score_003
WHERE pr_score_003 > 0.215
RETURN nid_003, pr_score_003
ORDER BY pr_score_003 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 35 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:BatchInference { priority: 4 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_4', {
  maxIterations: 29,
  dampingFactor: 0.943
}) YIELD nodeId AS nid_004, score AS pr_score_004
WITH nid_004, pr_score_004
WHERE pr_score_004 > 0.436
RETURN nid_004, pr_score_004
ORDER BY pr_score_004 DESC
LIMIT 20;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 17 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: false}
);

MATCH (start:BatchInference { priority: 3 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_5', {
  maxIterations: 23,
  dampingFactor: 0.892
}) YIELD nodeId AS nid_005, score AS pr_score_005
WITH nid_005, pr_score_005
WHERE pr_score_005 > 0.169
RETURN nid_005, pr_score_005
ORDER BY pr_score_005 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 3 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:BatchInference { priority: 2 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_9', {
  maxIterations: 21,
  dampingFactor: 0.89
}) YIELD nodeId AS nid_009, score AS pr_score_009
WITH nid_009, pr_score_009
WHERE pr_score_009 > 0.413
RETURN nid_009, pr_score_009
ORDER BY pr_score_009 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 38 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);

MATCH (start:BatchInference { priority: 8 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 3,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_10', {
  maxIterations: 13,
  dampingFactor: 0.87
...
CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 34 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 100, parallel: true}
);
...
CALL gds.pageRank.stream('batchinference_graph_11', {
  maxIterations: 35,
  dampingFactor: 0.705
}) YIELD nodeId AS nid_011, score AS pr_score_011
WITH nid_011, pr_score_011
WHERE pr_score_011 > 0.314
RETURN nid_011, pr_score_011
ORDER BY pr_score_011 DESC
LIMIT 50;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 1 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: false}
);

MATCH (start:BatchInference { priority: 7 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 6,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_14', {
  maxIterations: 20,
  dampingFactor: 0.903
}) YIELD nodeId AS nid_014, score AS pr_score_014
WITH nid_014, pr_score_014
WHERE pr_score_014 > 0.473
...
CALL gds.pageRank.stream('batchinference_graph_15', {
  maxIterations: 10,
  dampingFactor: 0.764
}) YIELD nodeId AS nid_015, score AS pr_score_015
WITH nid_015, pr_score_015
WHERE pr_score_015 > 0.327
RETURN nid_015, pr_score_015
ORDER BY pr_score_015 DESC
LIMIT 100;

CALL apoc.periodic.iterate(
  "MATCH (n:BatchInference) WHERE n.epoch = 43 RETURN n",
  "SET n.lastVisited = datetime(), n.visits = coalesce(n.visits, 0) + 1",
  {batchSize: 500, parallel: true}
);

MATCH (start:BatchInference { priority: 1 })
CALL apoc.path.subgraphAll(start, {
  maxLevel: 2,
  relationshipFilter: 'DEPENDS_ON>|TRIGGERS>',
  labelFilter: '+BatchInference'
...
CALL gds.pageRank.stream('batchinference_graph_17', {
  maxIterations: 48,
  dampingFactor: 0.845
}) YIELD nodeId AS nid_017, score AS pr_score_017
WITH nid_017, pr_score_017
WHERE pr_score_017 > 0.468
