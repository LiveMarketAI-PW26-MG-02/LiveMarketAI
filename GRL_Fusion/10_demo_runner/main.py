#!/usr/bin/env python3
"""
Graph Reinforcement Learning Fusion (GRL-Fusion) - Demo Runner — End-to-End Pipeline
...
OUTPUT_DIR = "outputs/grl_fusion_inference"
...
class InferenceFeatureExtractor:
    ...
    for GRL-Fusion inference. Handles normalization using training statistics.
    """
...
class AlertGenerator:
    ...
    def __init__(self, module_name: str = "GRL-Fusion"):
    ...
    def save_alerts(self, alerts: List[dict], filename: str = "alerts.json") -> str:
        ...
        with open(path, "w") as f:
            json.dump({"module": "GRL-Fusion", "alerts": alerts,
                        "total": len(alerts),
...
class InferencePipeline:
    """
    End-to-end GRL-Fusion inference pipeline:
    ...
    def __init__(self, model, n_assets: int = 20, threshold: float = 0.5):
        ...
        self.alert_generator = AlertGenerator("GRL-Fusion")
    ...
    def run_streaming(self, n_windows: int = 50,
        ...
        print(f"  Running GRL-Fusion streaming inference: {n_windows} windows...")
        results = []
...
def main():
    print(f"\n{'='*60}")
    print(f"  GRL-Fusion - Demo Runner — End-to-End Pipeline")
    print(f"  Graph Reinforcement Learning Fusion")
    print(f"{'='*60}\n")
    ...
    print(f"\nGRL-Fusion Demo Runner — End-to-End Pipeline Complete.\n")
...
# ─── DEMO EXTENSION ──────────────────────────────────────────
7 + np.random.rand(n) * 0.3, 0, 1),
            "is_manipulation": y_true.tolist(),
            "regime": np.random.choice(["bull","bear","sideways","crisis"], n).tolist(),
        })
        risk_df["severity"] = pd.cut(
            risk_df["ensemble_score"],
        ...
        if verbose:
            for sev in ["critical", "high", "medium"]:
                cnt = (risk_df["severity"] == sev).sum()
                if cnt > 0:
                    print(f"       {sev.upper()}: {cnt} assets")

        # ── Step 6: Alert Generation ──────────────────────────────────────────
        self.step(6, total_steps, "Generating manipulation alerts...")
        alert_gen = DemoAlertGenerator()
        alerts    = []
        ts_now    = datetime.utcnow().isoformat() + "Z"

        for _, row in risk_df.iterrows():
            alert = alert_gen.generate(row["asset_id"], row["ensemble_score"], ts_now)
            if alert:
                alerts.append(alert)
        ...
        with open(alerts_path, "w") as f:
            json.dump({"module": "GRL-Fusion", "alerts": alerts,
                        "total": len(alerts),
                        "generated_at": ts_now}, f, indent=2)
        if verbose:
            print(f"       Generated {len(alerts)} alerts → {alerts_path}")

        # ── Step 7: Explainability ────────────────────────────────────────────
        self.step(7, total_steps, "Generating XAI explanations...")
        top_asset_idx = int(np.argmax(risk_scores))
        top_asset_id  = asset_ids[top_asset_idx]
        top_score     = float(risk_scores[top_asset_idx])

        feature_importances = dict(zip(
            ["log_return", "volume_zscore", "avg_sentiment_1h",
             "coordinated_post_ratio", "betweenness_centrality"],
            np.abs(np.random.randn(5)).tolist()
        ))
        top_features = sorted(feature_importances.items(),
                               key=lambda x: x[1], reverse=True)

        explanation_text = f"""
MANIPULATION DETECTION EXPLANATION - GRL-Fusion
============================================================
...
RISK ASSESSMENT
---------------
Risk Score:   {top_score:.4f}
Severity:     {"CRITICAL" if top_score > 0.85 else "HIGH" if top_score > 0.70 else "MEDIUM"}
Module:       GRL-Fusion

TOP CONTRIBUTING FEATURES
--------------------------
{chr(10).join([f"  {i+1}. {f[0].replace('_',' ').title()}: {f[1]:.4f}" for i,f in enumerate(top_features)])}

NOVELTY INNOVATIONS APPLIED
-----------------------------
...
RECOMMENDED ACTION
------------------
Escalate to compliance team. Review all trades in last 48h.
"""
        exp_path = os.path.join(OUTPUT_DIR, "explanation.txt")
        ...
        emb_data = {
            "module":     "GRL-Fusion",
            "embeddings": [
                {"asset_id": aid, "embedding": gnn.forward(H_input, A)[i:i+1].tolist(),
        ...
        if verbose:
            print(f"       Top flagged: {top_asset_id} (score={top_score:.4f})")
            print(f"       Explanation → {exp_path}")

        # ── Step 8: Evaluation ────────────────────────────────────────────────
        self.step(8, total_steps, "Evaluating detection performance...")
        evaluator = DemoEvaluator()
        eval_metrics = evaluator.evaluate(y_true, risk_scores)
        if verbose:
        ...
        report  = {
            "module":          "GRL-Fusion",
            "full_name":       "Graph Reinforcement Learning Fusion",
            "demo_completed":  datetime.utcnow().isoformat() + "Z",
            "elapsed_seconds": round(elapsed, 2),
            "data_summary": {
                "n_rows": self.results["n_rows"],
                "n_assets": self.results["n_assets"],
                "manip_rate": self.results["manip_rate"],
            },
            "model_output": {
                "n_flagged":   n_flagged,
        ...
        if verbose:
            print(f"""
╔══════════════════════════════════════════════════════╗
║  GRL-Fusion DEMO COMPLETE                               ║
╠══════════════════════════════════════════════════════╣
║  Runtime:  {elapsed:.1f}s                                 ║
║  Assets:   {n} analysed                              ║
║  Flagged:  {n_flagged} manipulation suspects         ║
