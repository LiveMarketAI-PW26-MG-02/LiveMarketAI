#!/usr/bin/env python3
"""
Graph Reinforcement Learning Fusion (GRL-Fusion) - Risk Scoring Engine (Alt Entry)
Computes rl_fused_manipulation_score via ensemble of price, volume, sentiment, and graph scorers.
"""
...
OUTPUT_DIR = "outputs/grl_fusion_risk"
os.makedirs(OUTPUT_DIR, exist_ok=True)
np.random.seed(42)

class ComponentScorer:
    """Base component scorer for GRL-Fusion manipulation risk."""
...
class EnsembleRiskScorer:
    ...
    Module: GRL-Fusion | Primary risk metric: rl_fused_manipulation_score
    """
    ...
    def score_batch(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for scorer in self.scorers:
            try:
                ...
                pass
        result["rl_fused_manipulation_score"] = np.clip(ensemble, 0, 1)
        result["grl_fusion_severity"] = pd.cut(
            result["rl_fused_manipulation_score"], bins=self.SEVERITY_BINS, labels=self.SEVERITY_LABELS
        ).astype(str)
        result["grl_fusion_label"] = (result["rl_fused_manipulation_score"] >= 0.5).astype(int)
        return result
...
class RegimeAggregator:
    """Aggregates risk scores by market regime for GRL-Fusion."""
...
def main():
    ...
    print(f"  Mean {'rl_fused_manipulation_score'}: {scored_df['rl_fused_manipulation_score'].mean():.4f}")
    print(f"  High/Critical: {(scored_df['rl_fused_manipulation_score'] >= 0.7).sum()}")

    agg = RegimeAggregator()
    regime_stats = agg.aggregate(scored_df, 'rl_fused_manipulation_score')
    ...
    alerts = [
        {"alert_id": f"ALERT_{i:03d}", "asset_id": r["asset_id"],
          "risk_score": r["rl_fused_manipulation_score"], "severity": r["grl_fusion_severity"], "module": "GRL-Fusion"}
        for i, r in scored_df[scored_df["grl_fusion_label"]==1].iterrows()
    ]
    with open(os.path.join(OUTPUT_DIR, "alerts.json"), "w") as f:
        json.dump({"module":"GRL-Fusion","alerts":alerts}, f, indent=2)
    print(f"  Saved: risk_scores.csv + alerts.json ({len(alerts)} alerts)")
    print(f"\nGRL-Fusion Risk Scoring Complete.\n")
