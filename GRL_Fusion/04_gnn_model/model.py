#!/usr/bin/env python3
"""
Graph Reinforcement Learning Fusion (GRL-Fusion) - GNN Model — model.py Entry
Implements the GraphRLFusion graph neural network architecture
...
OUTPUT_DIR = "outputs/grl_fusion_model"
...
class GraphConvLayer:
    ...
    Used as a base building block in GRL-Fusion.
    """
...
class GraphAttentionLayer:
    ...
    Critical for GRL-Fusion to learn which graph neighbors matter most.
...
class RegimeAwareMessagePassing:
    ...
    Core innovation of GRL-Fusion: different regimes require different attention patterns.
    """
...
class TemporalGraphLayer:
    ...
    Captures evolving manipulation patterns over time in GRL-Fusion.
    """
...
class GraphRLFusion:
    """
    Graph Reinforcement Learning Fusion (GRL-Fusion) - Main GNN Model
    ...
    def forward(self, H: np.ndarray, A: np.ndarray,
        ...
        """
        Full forward pass through the GRL-Fusion GNN.
...
def build_model(config: Optional[dict] = None) -> GraphRLFusion:
    """Factory function to build GRL-Fusion model with default or custom config."""
    default_config = {
        "feature_dim":  32,
        "hidden_dim":   64,
        "n_layers":     3,
    ...
    model = GraphRLFusion(default_config)
    print(f"  Built GRL-Fusion model with config: {default_config}")
    return model
...
def main():
    print(f"\n{'='*60}")
    print(f"  GRL-Fusion - GNN Model Module")
    ...
    print(f"\nGRL-Fusion GNN Model Module Complete.\n")
    return model, output
