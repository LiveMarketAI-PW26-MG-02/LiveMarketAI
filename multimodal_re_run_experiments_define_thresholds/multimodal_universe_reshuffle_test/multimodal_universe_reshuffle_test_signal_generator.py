"""
  ...
  ZIP Topic  : ReRunExperiments
  Folder     : multimodal_universe_reshuffle_test
  File       : multimodal_universe_reshuffle_test_signal_generator.py
  Purpose    : Re Run Experiments — Universe Reshuffle Test
"""
...
MULTIMODAL_MODULE = 'multimodal_universe_reshuffle_test_signal_generator'
TOPIC             = 're_run_experiments'
VERSION           = '1.0.0'
DEFAULT_THRESHOLDS = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0]

# ── Configuration ─────────────────────────────────────────────────────────────
@dataclass
class MultimodalUniverseReshuffleTestSignalGeneratorConfig:
    """Multimodal threshold-sweep config for universe_reshuffle_test."""
...
# ── Core Engine ───────────────────────────────────────────────────────────────
class MultimodalUniverseReshuffleTestEngine(MultimodalThresholdBase):
    """
    Multimodal threshold engine: re run experiments — universe reshuffle test.
    Sweeps, evaluates, validates and documents signal thresholds.
    """
    def __init__(self, cfg: Optional[MultimodalUniverseReshuffleTestSignalGeneratorConfig] = None):
        self.cfg      = cfg or MultimodalUniverseReshuffleTestSignalGeneratorConfig()
